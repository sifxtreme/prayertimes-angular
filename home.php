	...
	.controller('prayerCtrl', ['$scope', '$http', '$timeout', '$interval', 'TimezoneSearcher', function($scope, $http, $timeout, $interval, TimezoneSearcher){	
		...
		$scope.$watch("place", function(newVal, oldVal){
			if(typeof newVal == "object"){
				...
				function(results){
					console.log(results);
					getPrayerTimes(d, $scope.latitude, $scope.longitude, results.data.rawOffset / 3600);
				},
				function(error){
					console.log("error");
					getPrayerTimes(d, $scope.latitude, $scope.longitude, prayerOffset);
				})
		...
		var getLocation = function(){
			if(navigator.geolocation){
				...
				navigator.geolocation.getCurrentPosition(function(position){
					...
					var offset = d.getTimezoneOffset();
					var prayerOffset = offset / 60 * -1;
					$scope.$apply(getPrayerTimes(d, $scope.latitude, $scope.longitude, prayerOffset));
				},
				function(error){
					console.log("ERROR");
					console.log(error);	
